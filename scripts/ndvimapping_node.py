...
class GridMapping(object):
    ...
    def pose_node_callback(self, data):
        ...
        tmp = pc2.read_points(cloud, skip_nans=True, field_names=("x", "y", "z", "ndvi"))
        tmp2 = []
        for x in tmp:
    ...
    def trajectory_callack(self, data):
        ...
        for i in range(it_len):
            ...
            for p in self.scans[x]:
                ...
                # Add to gridmap
                if not math.isnan(p[3]) and p[2] > 0.1: # Plants must be at least 10 cm large!
                    #print(p[2])
                    gmap.set_cell(transformed[0], transformed[1], (p[3] + 0.5) * 1.0, self.weight)
